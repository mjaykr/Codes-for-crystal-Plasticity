...
random_pairs=[];
theNumbers = randperm(size(all_euler,1)); % creating random no. between 1 to #grains
for k = 1 : 2 : size(theNumbers,1)
...
for i1 = 1:1:size(random_pairs,1)
    ...
    while iteration >0
        ...
        i3=2;
        pq = pq_lamel(euler_angle_pair);
        p = pq(1); q = pq(2);
        %% For Loop for calculation of New TWO euler angle Pair
        for i8=1:1:size(euler_angle_pair,1)
            ...
            while i7<=1
                ...
                if isempty(independent_slip_system)
                    ...
                    euler_angle_pair_collection(i8,:) = new_euler_angle;
                    i3=1;
                    i7=i7+1;
                end
            end
        end
        euler_angle_pair = euler_angle_pair_collection;
