function det_e456 = e4(euler_angle)
%% Iteration = 1
dummy6 =1;
...
det_e4=(-5) + 0.1*(index_min_taylor-1);
%% Iteration = 2
dummy6 =1;
Taylor_factor = zeros(1,round(((((det_e4+0.1)-(det_e4-0.1))/0.001)+1)));
m=zeros(1,56);
for e4=(det_e4-0.1):0.001:(det_e4+0.1)
    strain_external = [1 0 0;0 0 e4;0 e4 -1];
    g_mat = Euler_to_gmat(euler_angle);
    s_in= g_mat*strain_external*g_mat';
    for counter1=1:1:56 % Calculating m using bishop-hill table and picking max m as taylor factor 
...
[~, index_min_taylor] = min(Taylor_factor);
det_e4=(det_e4-0.1) + 0.001*(index_min_taylor-1);

%% Iteration = 3
dummy6 =1;
Taylor_factor = zeros(1,round(((((det_e4+0.001)-(det_e4-0.001))/0.00001)+1)));
m=zeros(1,56);
for e4=(det_e4-0.001):0.00001:(det_e4+0.001)
    strain_external = [1 0 0;0 0 e4;0 e4 -1];
    g_mat = Euler_to_gmat(euler_angle);
    s_in= g_mat*strain_external*g_mat';
    for counter1=1:1:56 % Calculating m using bishop-hill table and picking max m as taylor factor 
...
det_e4=(det_e4-0.001) + 0.00001*(index_min_taylor-1);




det_e456 = [Taylor_factor_min,det_e4];
end