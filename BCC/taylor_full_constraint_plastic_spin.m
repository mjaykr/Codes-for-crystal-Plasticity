...
common_slip =[];
all_rotation =[];
probable_independent_slip_system = [];
...
for counter1=1:1:56
    m(counter1) = round(sqrt(6)*( (bh(counter1,1)*s_in(2,2)) - ((bh(counter1,2)*s_in(1,1))) + ((bh(counter1,4)*2*s_in(2,3))) + ((bh(counter1,5)*2*s_in(3,1))) +((bh(counter1,6)*2*s_in(1,2)))),8);
end   
Taylor_factor = max(m);
for counter2=1:1:56
    if m(counter2)==max(m)
...
%         
% end
% probable_independent_slip_system = common_slip;
...
if isempty(collect_shear)
    euler_angle = euler_angle-1;
else

%% Finding Probable independent system
% 
for counter10=1:1:length(store_page_no_of_fivebyfive_mat)
%     
    location_of_ss = store_page_no_of_fivebyfive_mat(counter10);
    probable_independent_slip_system = Three_DM(:,:,location_of_ss);
    all_rotation(counter10)=net_rotation(probable_independent_slip_system,collect_shear,strain_increment); 
end

%% Finding min Net rotation 

[Net_rotation_min index_of_min_rotation] = min(all_rotation);
independent_slip_system =  Three_DM(:,:,store_page_no_of_fivebyfive_mat(index_of_min_rotation));
...
new_phi = real(round(radtodeg(round(acos(New_rotation_matrix(3,3)),6)),6));
new_phi1 = real(round(radtodeg(round(asin(New_rotation_matrix(3,1)/sin(degtorad(new_phi))),6)),6));
new_phi2 = real(round(radtodeg(round(acos(New_rotation_matrix(2,3)/round(sin(degtorad(new_phi)),3)),6)),6));
new_euler_angle = round([new_phi1 new_phi new_phi2],3);
...
simulated_euler_angles = store_euler_angles