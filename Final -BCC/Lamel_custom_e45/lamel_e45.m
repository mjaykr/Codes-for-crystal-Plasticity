...
for i1 = 1:1:size(random_pairs,1)
    ...
    while iteration >0
        i3=1;
        e_vector = calc_e45(euler_angle_pair); % Output is in vector form [e4 e5 e6]
        e4 = e_vector(1); % Selecting only e4 out of e_Vector
        e5 = e_vector(2); % Selecting only e4 out of e_Vector
        for i8=1:1:size(euler_angle_pair,1)
            euler_angle = euler_angle_pair(i8,:);
            strain_external = [1 0 e5*i3;0 0 e4*i3;e5*i3 e4*i3 -1];
